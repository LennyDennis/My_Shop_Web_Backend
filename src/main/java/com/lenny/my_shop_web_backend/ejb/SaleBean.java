...
import com.lenny.my_shop_web_backend.ejb_db.*;
import com.lenny.my_shop_web_backend.entities.*;
import com.lenny.my_shop_web_backend.jpa.TransactionProvider;
import com.lenny.my_shop_web_backend.models.CartProduct;
...
public class SaleBean {
    ...
    @EJB
    OutOfStockDatabaseBean outOfStockDatabaseBean;
    ...
    public Response createSell(CreateSale newSale) {
        try {
            ...
            List<CartProduct> cartProducts = newSale.getSaleProductDetails();
            if (!cartProducts.isEmpty()) {
                for (CartProduct cartProduct : cartProducts) {
                    SaleDetail saleDetail = new SaleDetail();
                    saleDetail.setProduct(cartProduct.getCartProductId());
                    saleDetail.setSellingPrice(cartProduct.getCartSellingPrice());
                    saleDetail.setSoldQuantity(cartProduct.getCartQuantityToSell());
        ...
        for (SaleDetail saleDetail : saleDetails) {
            ...
            if (currentStockQuantity >= MIN_RESTOCK_ACTIVATION_NUMBER) {
                if (newStockQuantity < MIN_RESTOCK_ACTIVATION_NUMBER) {
                    ...
                    List<User> allAdmins = userDatabaseBean.getAllAdmins();
                    if (allAdmins.isEmpty()) {
                        throw new PersistenceException("No admin exist. Can't update stock");
                    }
                    List<Notification> updateNotificationList = new ArrayList();
                    List<Notification> newNotificationList = new ArrayList();
                    for (User user : allAdmins) {
                        Notification notificationExist = outOfStockDatabaseBean.getNotification_ByProductAndUser(user, product);
                        if (notificationExist != null) {
                            notificationExist.setViewStatus(NOT_VIEWED);
                            updateNotificationList.add(notificationExist);
                        }else {
                            Notification notification = new Notification();
                            notification.setUser(user);
                            notification.setProduct(product);
                            notification.setDate(currentDate);
                            notification.setViewStatus(NOT_VIEWED);
                            newNotificationList.add(notification);
                        }
                    }
                    if(!updateNotificationList.isEmpty()){
                        if(!provider.updateMultipleEntities(updateNotificationList)){
                            throw new PersistenceException("Notification not updated successfully");
                        }
                    }
                    if(!newNotificationList.isEmpty()){
                        if(!provider.createMultipleEntities(newNotificationList)){
                            throw new PersistenceException("Notification not added successfully");
                        }
                    }
                }
            }
...
//            }
        ...
        for (Sale sale : sales) {
            ...
            if (!saleDetails.isEmpty()) {
                for (SaleDetail saleDetail : saleDetails) {
                    ...
                    salesDetailHashMap.put("totalCost", assignTwoDecimal(saleDetail.getSellingPrice()*saleDetail.getSoldQuantity()));
                    ...
                    salesDetailHashMap.put("sellDate", getDateFromMilliSeconds(sale.getSellDate()));
    ...
    private void getSaleInfo(Sale sale, List<HashMap<String, Object>> saleInfoList) {
        ...
        saleHashMap.put("modifiedOn", getDateFromMilliSeconds(sale.getModifiedOn()));
        saleHashMap.put("sellDate", getDateFromMilliSeconds(sale.getSellDate()));
