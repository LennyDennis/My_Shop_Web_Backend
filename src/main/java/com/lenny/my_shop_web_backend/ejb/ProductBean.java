...
import com.lenny.my_shop_web_backend.ejb_db.OutOfStockDatabaseBean;
...
//            }
    ...
    public Response getOutOfStockProducts() {
        try {
            List<Product> outOfStockProducts = outOfStockDatabaseBean.getOutOfStockProducts();
            HashMap<String, Object> res = new HashMap<>();
    ...
    public Response getOutOfStockNotifications() {
        try {
            List<Notification> notificationList = outOfStockDatabaseBean.getOutOfStockNotification();
            List<Product> outOfStockProducts = new ArrayList<>();
            HashMap<String, Object> res = new HashMap<>();
            if (notificationList.isEmpty()) {
                res.put("message", "No out of stock notification exists");
            } else {
                for (Notification notification : notificationList) {
                    outOfStockProducts.add(notification.getProduct());
                }
                List<Object> outOfStockList = new ArrayList<>();
                saveProductInHashMap(outOfStockProducts, outOfStockList);
                res.put("products", outOfStockList);
                res.put("message", "Out of stock notifications fetched successfully");
            }
    ...
    public Response clearOutOfStockNotifications(List<Product> products) {
        try {
            HashMap<String, Object> res = new HashMap<>();
            if(products.isEmpty()){
                throw new BadRequestException("Out of stock notification products is empty");
            }
            for(Product product:products){
                List<Notification> notifications = outOfStockDatabaseBean.getNotViewedNotifications_ByProduct(product);
                if(!notifications.isEmpty()){
                    for(Notification notification:notifications){
                        notification.setViewStatus(VIEWED);
                    }
                    if(!transactionProvider.updateMultipleEntities(notifications)){
                        throw new PersistenceException("Notifications not updated successfully");
                    }
                }
            }
            res.put("message", "Notifications cleared successfully");
            return Response.status(Response.Status.OK).entity(res).build();
        } catch (BadRequestException e) {
            return Response.status(Response.Status.BAD_REQUEST).entity(e.getMessage()).build();
        } catch (PersistenceException e) {
    ...
    public Response restockProduct(Integer productId, Integer newStockQuantity) {
        try {
            ...
            if (newStock >= MIN_RESTOCK_ACTIVATION_NUMBER && restockStatus.equals(RESTOCK_STATUS_ON)) {
                ...
                updateNotificationToViewed(product, currentDate);
            }
    ...
    private void updateNotificationToViewed(Product product, Date currentDate) {
        List<Notification> notificationList = outOfStockDatabaseBean.getNotViewedNotifications_ByProduct(product);
        if (!notificationList.isEmpty()) {
            for (Notification notification : notificationList) {
                notification.setViewStatus(VIEWED);
                notification.setDate(currentDate);
            }
            if (!transactionProvider.updateMultipleEntities(notificationList)) {
